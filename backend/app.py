...
CORS(app, origins=allowed_origins, supports_credentials=True, allow_headers=[
    'Content-Type', 'Authorization', 'user_insipredict_id', 'first_name_insipredict_user'
])
...
def generate_tailwind_table(df):
    """Generate a Tailwind-styled HTML table for DataFrame preview."""
    html = '<table class="w-full bg-gray-900 text-gray-100 rounded-2xl overflow-hidden shadow-lg">'
    html += '<thead><tr>'
    for col in df.columns:
        html += f'<th class="px-6 py-4 text-left font-semibold text-white border-b border-gray-700">{col}</th>'
...
@app.route('/debug-session', methods=['GET', 'OPTIONS'])
def debug_session():
    """Debug endpoint to check session state."""
    if request.method == 'OPTIONS':
        response = jsonify({'status': 'ok'})
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("⚠️  Continuing without auth functionality...")
    
    print("🚀 Starting Flask CSV Analysis Chatbot with Authentication...")
    print("📊 Backend running on http://localhost:5000")
