...
#         analyzer = StreamingAnalyzer(new_session_id, socketio)
...
#             try:
#                 from utils import generate_tailwind_table
...
#     """Terminate entire session immediately."""
#     if request.method == 'OPTIONS':
#         response = jsonify({'status': 'ok'})
#         origin = request.headers.get('Origin', '*')
#         response.headers.add('Access-Control-Allow-Origin', origin)
...
#     try:
#         # Clean up session data using utility function
#         deleted_items = cleanup_session_data(session_id, session_data, analyzers)
...
#     """Clean up old inactive sessions with improved logic."""
#     if request.method == 'OPTIONS':
#         response = jsonify({'status': 'ok'})
#         origin = request.headers.get('Origin', '*')
#         response.headers.add('Access-Control-Allow-Origin', origin)
...
#         force_cleanup = data.get('forceCleanup', False)
...
#                 elif is_session_too_old(session_info, max_age_hours):
#                     should_delete = True
#                     reason = "too_old"
#                 elif is_session_inactive(session_info, max_inactive_hours):
#                     should_delete = True
#                     reason = "inactive"
...
#                         'conversationCount': len(analyzers[session_id].conversation_history.history) if session_id in analyzers else 0
#                     })
...
#                 deleted_items = cleanup_session_data(session_id, session_data, analyzers)
#                 if deleted_items:
#                     session_info['deletedItems'] = deleted_items
#                     deleted_sessions.append(session_info)
#                     print(f"Cleaned up session: {session_id} ({session_info['reason']})")
...
#         updated_stats = get_session_stats(session_data, analyzers)
...
#             },
#             'updatedStatistics': updated_stats,
...
#             deleted_items = cleanup_session_data(current_session_id, session_data, analyzers)
#             if deleted_items:
#                 print(f"Cleared session: {current_session_id}")
...
#             'cleared_items': deleted_items if current_session_id else []
#         })
...
#         'server_stats': get_session_stats(session_data, analyzers)
#     }
...
# @socketio.on('connect')
# def handle_connect():
#     """Handle client connection with improved session management and conversational features."""
...
#         has_data = session_id in analyzers and analyzers[session_id].df is not None
...
#     """Handle chat messages for legacy support."""
#     session_id = session.get('session_id')
...
#     # Redirect to the session-specific handler
...
#     """Enhanced chat message handler with query classification and improved routing."""
#     session_id = data.get('sessionId')
...
#     # For conversational queries, we don't need to validate data loading
#     # For analytical queries, we need to validate session exists and has data
    
#     # First, try to classify the query to determine routing
#     try:
#         # Create a temporary analyzer instance for classification if needed
#         if session_id in analyzers:
#             analyzer = analyzers[session_id]
#         else:
#             # For conversational queries, we can create a minimal analyzer
#             analyzer = None
        
#         # Simple classification logic (can be moved to utils if needed)
#         query_lower = query.lower().strip()
#         conversational_patterns = [
#             r'^(hi|hello|hey|good morning|good afternoon|good evening|hiya)(?:\s|$)',
#             r'how are you', r'how\'s it going', r'what\'s up',
#             r'^(thanks|thank you|ty|thx)', r'who are you', r'what are you',
#             r'what can you do', r'^help(?:\s|$)', r'goodbye|bye'
#         ]
        
#         is_conversational = any(re.search(pattern, query_lower) for pattern in conversational_patterns)
        
#         # If it's clearly conversational, handle without data validation
#         if is_conversational:
#             print(f"💬 Detected conversational query: {query[:50]}...")
            
#             # Create minimal analyzer for conversational handling if doesn't exist
#             if session_id not in analyzers:
#                 # Create basic analyzer without data for conversational queries
#                 analyzer = StreamingAnalyzer(session_id, socketio)
#                 analyzers[session_id] = analyzer
#                 # Also create minimal session data if needed
...
#                         'data': 'Conversational query detected',
...
#                         socketio.emit('stream_data', {
#                             'type': 'conversational_complete',
#                             'data': result.get('ai_response', ''),
#                             'timestamp': datetime.now().isoformat(),
#                             'sessionId': session_id,
...
#                         socketio.emit('stream_data', {
#                             'type': 'completion',
#                             'data': 'Conversational response completed',
#                             'timestamp': datetime.now().isoformat(),
#                             'sessionId': session_id,
#                             'result': result
#                         }, room=session_id)
...
#                     print(f"Conversational query stopped by user for session {session_id}")
#                     socketio.emit('stream_data', {
#                         'type': 'stopped',
#                         'data': '⏹️ Conversation stopped by user',
#                         'timestamp': datetime.now().isoformat(),
#                         'sessionId': session_id
#                     }, room=session_id)
                    
#                 except Exception as e:
#                     print(f"Conversational query error for session {session_id}: {str(e)}")
#                     socketio.emit('stream_data', {
#                         'type': 'error',
#                         'data': f'Conversational response failed: {str(e)}',
#                         'timestamp': datetime.now().isoformat(),
#                         'sessionId': session_id
#                     }, room=session_id)
            
#             # Run in thread
#             thread = threading.Thread(target=process_conversational_query)
#             thread.daemon = True
#             thread.start()
#             return
    
#     except Exception as e:
#         print(f"⚠️ Query classification error: {e}")
#         # Continue with normal processing if classification fails
    
#     # For analytical queries, validate session exists and has data
#     is_valid, error_msg = validate_session_exists(session_id, analyzers, session_data)
#     if not is_valid:
#         emit('stream_data', {
#             'type': 'error',
#             'data': f'{error_msg}. Please go back to home and upload a file for data analysis.',
#             'timestamp': datetime.now().isoformat(),
#             'suggestion': 'Upload a CSV file to enable data analysis features'
#         })
#         return
...
#     # Process analytical query with session-specific analyzer
#     def process_analytical_query():
#         try:
#             analyzer = analyzers[session_id]
#             analyzer.session_id = session_id
            
#             # Emit classification and starting analysis
#             socketio.emit('stream_data', {
#                 'type': 'query_classified',
#                 'data': 'Analytical query detected - starting data analysis',
...
#                 'data': f'Starting analysis for: {query}',
#                 'timestamp': datetime.now().isoformat(),
#                 'sessionId': session_id
#             }, room=session_id)
...
#     thread = threading.Thread(target=process_analytical_query)
#     thread.daemon = True
#     thread.start()
...
#     """Handle request to stop analysis or terminate session."""
#     session_id = data.get('sessionId')
#     stop_type = data.get('type', 'query')  # 'query' or 'session'
    
#     if not session_id:
#         emit('error', {'message': 'No session ID provided for stop request'})
#         return
    
#     if stop_type == 'session':
#         print(f"Session termination requested for: {session_id}")
...
#         emit('stream_data', {
#             'type': 'session_terminated',
#             'data': 'Session being terminated by user...',
#             'timestamp': datetime.now().isoformat(),
#             'sessionId': session_id
#         }, room=session_id)
        
#         # Clean up session after brief delay
#         def delayed_cleanup():
#             import time
#             time.sleep(1)
#             cleanup_session_data(session_id, session_data, analyzers)
#             print(f"Session {session_id} terminated via WebSocket")
...
#     else:
#         print(f"Query stop requested for session: {session_id}")
        
#         # Set stop signal for current query only
#         stop_analysis_for_session(session_id)
...
#     """Handle request to completely terminate a session."""
#     session_id = data.get('sessionId')
    
#     if not session_id:
#         emit('error', {'message': 'No session ID provided for termination'})
#         return
    
#     print(f"Session termination requested via WebSocket: {session_id}")
    
#     # Stop any running analysis
#     stop_analysis_for_session(session_id)
    
#     # Emit termination signal to all clients in the room
#     socketio.emit('stream_data', {
#         'type': 'session_terminated',
#         'data': 'Session terminated by user',
#         'timestamp': datetime.now().isoformat(),
...
#     # Emit final cleanup confirmation
...
# @socketio.on('classify_query')
# def handle_classify_query(data):
#     """Handle explicit query classification request."""
#     session_id = data.get('sessionId')
#     query = data.get('query', '').strip()
    
#     if not session_id or not query:
#         emit('error', {'message': 'Session ID and query required for classification'})
#         return
    
#     try:
#         # Create temporary analyzer for classification if needed
#         if session_id in analyzers:
#             analyzer = analyzers[session_id]
#             query_type = analyzer._classify_query(query)
#         else:
#             # Simple classification without analyzer
#             from utils import StreamingAnalyzer
#             temp_analyzer = StreamingAnalyzer(session_id)
#             query_type = temp_analyzer._classify_query(query)
        
#         emit('query_classified', {
#             'sessionId': session_id,
#             'query': query,
...
#             'capabilities': {
#                 'conversational': True,
#                 'analytical': session_id in analyzers and analyzers[session_id].df is not None
#             }
#         })
...
# @socketio.on('get_session_capabilities')
# def handle_get_session_capabilities(data):
...
#     if session_id in analyzers:
#         analyzer = analyzers[session_id]
#         has_langchain = getattr(analyzer.conversation_history, 'langchain_enabled', False)
    
#     capabilities = {
#         'sessionId': session_id,
#         'conversational': True,  # Always available
#         'analytical': has_data,
#         'query_classification': True,
#         'langchain_memory': has_langchain,
...
#     if has_data and session_id in session_data:
#         capabilities['data_info'] = {
#             'filename': session_data[session_id].get('filename'),
#             'shape': session_data[session_id].get('shape'),
#             'columns': len(session_data[session_id].get('columns', []))
#         }
    
#     emit('session_capabilities', capabilities)
...
#     print("💬 Conversational queries: Enabled")
...
@app.route('/upload', methods=['POST', 'OPTIONS'])
def upload_file_with_session():
    """Handle CSV file upload and create a new session - UPDATED TO USE ENHANCED ANALYZER."""
    if request.method == 'OPTIONS':
        response = jsonify({'status': 'ok'})
        origin = request.headers.get('Origin', '*')
        response.headers.add('Access-Control-Allow-Origin', origin)
    ...
    try:
        ...
        # Initialize ENHANCED analyzer for this session
        analyzer = EnhancedStreamingAnalyzer(new_session_id, socketio)
        
        # Load the CSV (this will initialize all handlers)
        if analyzer.load_csv(filepath):
            analyzers[new_session_id] = analyzer
            session_data[new_session_id] = {
                'filename': file.filename,
                ...
                'created_by': session.get('user_id', 'anonymous'),
                'last_activity': datetime.now().isoformat(),
                'analyzer_type': 'enhanced'  # Track that this is an enhanced analyzer
            }
            
            # Clear any existing stop signals for this session
            clear_stop_signal_for_session(new_session_id)
            
            print(f"✅ Created new ENHANCED session: {new_session_id} for file: {file.filename}")
            
            response = jsonify({
                'success': True,
                'sessionId': new_session_id,
                'message': f'File uploaded successfully! Enhanced analyzer ready. Shape: {analyzer.df.shape}',
                'data': {
                    'filename': file.filename,
                    'shape': analyzer.df.shape,
                    'columns': list(analyzer.df.columns),
