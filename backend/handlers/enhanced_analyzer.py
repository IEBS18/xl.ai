...
#             'other': []
#         }
#          # NEW: Ensure local output directory exists
#         self._ensure_output_directory_exists()
    
#     # NEW: Track local report paths for this session
#         self.local_report_paths = []    
#         logging.info(f"✅ Enhanced analyzer with Assistants API initialized for session: {session_id}")
...
#             logging.info(f"✅ File loaded successfully from blob storage!")
...
#         except Exception as e:
#             logging.info(f"❌ Error loading file from SAS URL: {str(e)}")
...
#             logging.info(f"✅ CSV loaded and handlers initialized for enhanced analysis")
...
#             logging.info("✅ All query handlers initialized with Assistants API")
            
#         except Exception as e:
#             logging.info(f"⚠️ Failed to initialize some handlers: {e}")
...
#             logging.info(f"📋 Query classified as: {query_category}")
#             logging.info(f"🔍 Metadata: {classification_metadata}")
...
#             logging.info("⚠️ DataFrame utils not available, using local implementation")
...
#             logging.info("⚠️ DataFrame validation failed, using fallback formatting")
...
#             logging.info("⚠️ Code validation failed, using fallback formatting")
...
#         except Exception as e:
#             logging.info(f"Error generating DataFrame preview: {e}")
...
#             logging.info(f"⚠️ Unknown category '{category}', falling back to original analysis")
...
#         logging.info("📊 Handling textual analytical query with Assistants API")
...
#         except Exception as e:
#             logging.info(f"❌ Assistants textual analytical handler failed: {e}")
#             return self._fallback_textual_analytical_handler(user_query, intent_data)
...
#         """UPDATED: Download HTML report from sandbox and save locally"""
#         try:
#             # Extract run information
#             run_id = result.get("run_id")
#             thread_id = result.get("thread_id", self.thread_id)
            
#             if not run_id:
#                 logging.info("⚠️ No run_id found in result, cannot download from sandbox")
#                 return None
            
#             logging.info(f"🔍 Downloading HTML report from sandbox - Run: {run_id}")
            
#             # Use assistant manager to download reports
#             download_result = self.assistant_manager.download_all_reports_to_local(
#                 thread_id=thread_id,
#                 run_id=run_id,
#                 output_directory="backend/output"
#             )
            
#             if download_result.get("success") and download_result.get("downloaded_reports"):
#                 # Get the first downloaded report
#                 primary_report = download_result["downloaded_reports"][0]
#                 local_html_path = primary_report["local_path"]
...
#                     logging.info(f"✅ Successfully downloaded HTML report: {local_html_path}")
#                     logging.info(f"📄 Report size: {len(html_content):,} characters")
                    
#                     return html_content
                    
#                 except Exception as read_error:
#                     logging.info(f"❌ Could not read downloaded HTML file: {read_error}")
#                     return None
#             else:
#                 logging.info(f"⚠️ No HTML reports downloaded: {download_result.get('message', 'Unknown reason')}")
#                 return None
                
#         except Exception as e:
#             logging.info(f"❌ Error downloading HTML report from sandbox: {e}")
#             return None

#     def _get_local_report_path(self, result: Dict[str, Any]) -> str:
#         """NEW: Get the local path of downloaded HTML report"""
#         try:
#             run_id = result.get("run_id")
#             thread_id = result.get("thread_id", self.thread_id)
...
#             # Use assistant manager to get download info
#             download_result = self.assistant_manager.download_all_reports_to_local(
#                 thread_id=thread_id,
#                 run_id=run_id,
#                 output_directory="backend/output"
...
#             if download_result.get("success") and download_result.get("primary_report_path"):
#                 return download_result["primary_report_path"]
#             else:
#                 return ""
                
#         except Exception as e:
#             logging.info(f"❌ Error getting local report path: {e}")
#             return ""

#     def _ensure_output_directory_exists(self) -> bool:
#         """NEW: Ensure the backend/output directory exists"""
#         try:
...
#         """UPDATED: Handle analytical queries with local HTML report download from sandbox"""
        
#         logging.info("🔬 Handling analytical query with local HTML report download")
...
#             CRITICAL: You MUST save the complete HTML report with embedded images to /mnt/data/professional_analysis_report.html
#             """
...
#             if result.get("success"):
#                 # ENHANCED: Download HTML reports from sandbox to local directory
#                 self.emit_stream('status', '📥 Downloading HTML report from assistant sandbox...')
                
#                 # Download reports to local backend/output directory
#                 download_result = self.assistant_manager.download_all_reports_to_local(
#                     thread_id=self.thread_id,
#                     run_id=run.id,
#                     output_directory="backend/output"
...
#                 # Download and categorize other generated files (images, etc.)
...
#                 # Check if we successfully downloaded HTML report
#                 local_html_report = None
#                 report_url = None
...
#                         with open(local_html_path, 'r', encoding='utf-8') as f:
#                             local_html_report = f.read()
                        
#                         # Stream the HTML report to frontend
#                         self.emit_stream('report', local_html_report)
                        
#                         # Generate a URL for the frontend to access the report
#                         report_filename = primary_report["filename"]
#                         report_url = f"/reports/{report_filename}"  # This will be served by app.py
                        
#                         logging.info(f"✅ Successfully downloaded and loaded HTML report: {local_html_path}")
                        
#                     except Exception as read_error:
#                         logging.info(f"⚠️ Could not read downloaded HTML file: {read_error}")
#                         local_html_report = None
                
#                 # Fallback: Try to extract HTML from assistant response if download failed
#                 if not local_html_report:
#                     logging.info("⚠️ Local HTML download failed, extracting from response...")
#                     local_html_report = self._extract_html_from_assistant_response(result.get("response_content", ""))
#                     self.emit_stream('report', local_html_report)
...
#                     "comprehensive_report": local_html_report,  # LOCAL HTML WITH EMBEDDED IMAGES
#                     "report_generated": True,
#                     "local_report_downloaded": download_result.get("success", False),
#                     "local_report_path": download_result.get("primary_report_path", ""),
#                     "report_url": report_url,  # URL for frontend access
#                     "download_info": download_result,  # Full download metadata
#                     "generated_code": result.get("generated_code", ""),
#                     "execution_result": {
#                         "success": True,
...
#                     "report_source": "local_download" if download_result.get("success") else "response_extraction"
#                 }
#             else:
#                 return self._fallback_fully_analytical_handler(user_query, intent_data)
                
#         except Exception as e:
#             logging.info(f"❌ Enhanced HTML report generation failed: {e}")
#             return self._fallback_fully_analytical_handler(user_query, intent_data)
...
#                 logging.info("⚠️ Blob storage not configured, cannot upload HTML report")
#                 return ""
...
#                 logging.info(f"✅ HTML report uploaded to: {report_url}")
#                 return report_url
#             else:
#                 logging.info(f"✅ HTML report uploaded to: {blob_client.url}")
#                 return blob_client.url
                
#         except Exception as e:
#             logging.info(f"❌ Error uploading HTML report to blob: {e}")
#             return ""
...
#                     logging.info("✅ Extracted HTML report from assistant response")
#                     return html_content
            
#             # If no HTML found in response, create wrapper with the content
#             logging.info("⚠️ No HTML found, creating simple report wrapper")
#             return f"""
#             <!DOCTYPE html>
#             <html lang="en">
#             <head>
...
#         except Exception as e:
#             logging.info(f"❌ Error extracting HTML: {e}")
...
#         except Exception as e:
#             logging.info(f"⚠️ Error extracting DataFrames from assistant result: {e}")
...
#         except Exception as e:
#             logging.info(f"⚠️ Error parsing DataFrame from output: {e}")
...
#         logging.info("📋 Handling report generation query")
...
#         except Exception as e:
#             logging.info(f"❌ Report generation failed: {e}")
#             return {
#                 "query": user_query,
#                 "type": "report",
#                 "success": False,
...
#         except Exception as e:
#             logging.info(f"❌ Error generating HTML report: {e}")
...
#         except Exception as e:
#             logging.info(f"⚠️ Failed to generate explanation: {e}")
...
#         logging.info("🔄 Falling back to original analysis method")
...
#         except Exception as e:
#             logging.info(f"❌ Even original analysis failed: {e}")
#             return {
#                 "error": str(e),
#                 "type": "fallback_error",
#                 "success": False,
...
#             # NEW: Clean up local reports (optional - keep last 5 for reference)
#             try:
#                 deleted_count = self.cleanup_old_reports(keep_last_n=5)
#                 if deleted_count > 0:
#                     logging.info(f"🧹 Cleaned up {deleted_count} old HTML reports")
#             except Exception as cleanup_error:
#                 logging.warning(f"⚠️ Could not clean up local reports: {cleanup_error}")
            
#             logging.info(f"🧹 Cleaned up assistants resources and local files for session: {self.session_id}")
...
#         """Get information about analysis capabilities - UPDATED with local report info"""
...
#             },
#             "report_generation": {
#                 "description": "Generate complete HTML reports downloaded locally",
#                 "examples": ["Create a report", "Generate executive summary", "Make comprehensive analysis report"],
#                 "powered_by": "OpenAI Assistants API + Local Download System"
#             },
#             "local_reports": {
#                 "description": "HTML reports are downloaded and stored locally for reliable access",
#                 "examples": ["Reports available at /reports/{filename}", "No dependency on sandbox lifecycle"],
#                 "powered_by": "Enhanced Local File Management"
#             },
#             "dataframe_results": {
...
#         # NEW: Add local reports info
#         try:
#             local_reports = self.list_local_reports()
#             session_reports = self.get_session_reports()
            
#             capabilities["local_storage"] = {
#                 "output_directory": "backend/output",
#                 "total_reports": len(local_reports),
#                 "session_reports": len(session_reports),
#                 "latest_reports": [r["filename"] for r in local_reports[:3]]  # Show latest 3
#             }
#         except Exception as e:
#             capabilities["local_storage"] = {
#                 "error": str(e),
...
#     def get_local_report_url(self, filename: str) -> str:
#         """NEW: Generate URL for accessing local reports via frontend"""
#         try:
#             # Check if file exists in backend/output
#             local_path = Path("backend/output") / filename
#             if local_path.exists():
#                 return f"/reports/{filename}"
#             else:
#                 return ""
#         except Exception as e:
#             logging.info(f"❌ Error generating report URL: {e}")
#             return ""

#     def list_local_reports(self) -> List[Dict[str, Any]]:
#         """NEW: List all local HTML reports in the output directory"""
#         try:
#             reports = []
#             output_dir = Path("backend/output")
...
#                         logging.info(f"⚠️ Error reading file info for {html_file}: {file_error}")
...
#             return reports
#         except Exception as e:
#             logging.info(f"❌ Error listing local reports: {e}")
#             return []

#     def cleanup_old_reports(self, keep_last_n: int = 10) -> int:
#         """NEW: Clean up old HTML reports, keeping only the latest N files"""
#         try:
#             reports = self.list_local_reports()
...
#                     deleted_count += 1
#                     logging.info(f"🗑️ Deleted old report: {report['filename']}")
#                 except Exception as delete_error:
#                     logging.info(f"⚠️ Could not delete {report['filename']}: {delete_error}")
            
#             return deleted_count
#         except Exception as e:
#             logging.info(f"❌ Error cleaning up old reports: {e}")
#             return 0

#     def get_session_reports(self) -> List[Dict[str, Any]]:
#         """NEW: Get reports generated in this session"""
#         try:
#             session_reports = []
            
#             # Look for reports that contain this session ID in filename or were created recently
#             all_reports = self.list_local_reports()
            
#             for report in all_reports:
#                 # Check if report was created in the last hour (likely from this session)
#                 created_time = datetime.fromisoformat(report["created"])
...
#         except Exception as e:
#             logging.info(f"❌ Error getting session reports: {e}")
#             return []
...
class EnhancedStreamingAnalyzer(StreamingAnalyzer):
    ...
    def _handle_fully_analytical_query_with_assistants(self, user_query: str, intent_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            report_result = self._generate_plain_text_report_with_images(
            ...
            if report_result.get("success"):
                plain_text_report = report_result.get("plain_text_report", "")
                
                # Stream the plain text report to frontend
                self.emit_stream('report', {
                ...
                print("✅ Successfully generated comprehensive report with embedded images")
                ...
                return {
                    ...
                    "comprehensive_report": plain_text_report,  # Plain text with embedded image URLs
                    "report_generated": True,
                    "report_type": "plain_text_with_images",
                    ...
                    "same_thread_analysis": True
                }
            else:
                # Report generation failed, return analysis results only
                print("⚠️ Report generation failed, returning analysis results")
