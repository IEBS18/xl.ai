...
class QuadraticCSVAnalyzer:
    """
    Enhanced Quadratic-inspired CSV analyzer with conversation history management.
    """
    
    def __init__(self):
        """Initialize the analyzer with Azure OpenAI client and conversation history."""
    ...
    def _setup_directories(self):
        ...
        self.conversation_history = ConversationHistory(self.current_session_id, self.output_dir)
        
        print(f"📁 Output directory created: {self.output_dir}")
        print(f"📜 Conversation history initialized for session: {self.current_session_id}")
    ...
    def load_csv(self, file_path: str) -> bool:
        ...
        try:
            ...
            # Log file loading in conversation history
            self.conversation_history.add_conversation(
                f"Load CSV file: {file_path}",
                {
                    "type": "file_load",
                    "success": True,
                    "dataframes": {"main": self.df}
                }
            )
            ...
            # Log error in conversation history
            self.conversation_history.add_conversation(
                f"Load CSV file: {file_path}",
                {
                    "type": "file_load",
...
"""
    ...
    def _create_system_prompt(self) -> str:
        """Create system prompt with conversation history context."""
        conversation_context = self.conversation_history.get_context_for_ai()
    ...
    def analyze_query(self, user_query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_report_request:
                ...
                result = self._generate_comprehensive_report(user_query, is_forecasting)
            else:
                # Focus on DataFrame results
                result = self._generate_dataframe_analysis(user_query, data_request, is_forecasting)
...
"""
    ...
    def show_session_summary(self):
        """Display session summary with conversation history."""
        ...
        print(f"📸 Generated Images: {len(self.generated_images)}")
        print("="*60)
    ...
    def interactive_session(self):
        ...
        print("📜 Conversation history is automatically tracked")
        print("\nExamples:")
        ...
        while True:
            try:
                ...
                if query.lower() == 'history':
                    print("\n📜 CONVERSATION HISTORY:")
                    for i, conv in enumerate(self.conversation_history.history, 1):
                        print(f"\n{i}. {conv['timestamp'][:19]}")
                        print(f"   Query: {conv['user_query']}")
                        print(f"   Type: {conv['response_type']}")
                        print(f"   Status: {'✅' if conv['success'] else '❌'}")
                        if conv.get('error'):
                            print(f"   Error: {conv['error'][:50]}...")
                    continue
