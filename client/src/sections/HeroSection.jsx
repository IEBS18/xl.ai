...
//                                 <Database size={12} />
//                                 <div>
//                                   <div className="text-xs font-medium">Connect Database</div>
...
import React, { useState, useRef, useEffect } from "react"
import { Send, Plus, Upload, Database, Sparkles, FileText } from "lucide-react"
...
const HeroSection = ({ isConnected, onSendMessage, onFileUpload, onDatabaseConnect, uploadProgress = 0 }) => {
  ...
  useEffect(() => {
    if (showDataSourceDropdown && buttonRef.current) {
      const rect = buttonRef.current.getBoundingClientRect()
      setDropdownPosition({
        top: rect.bottom + 8,
        left: rect.right - 250 // 250px is dropdown width
      })
    }
  }, [showDataSourceDropdown])
  ...
  return (
    <>
      ...
      {/* Portal Dropdown - Renders at document root */}
      {showDataSourceDropdown && isAuthenticated && typeof document !== 'undefined' && ReactDOM.createPortal(
        <div 
          ref={dropdownRef}
          className={`fixed min-w-[250px] ${isDark 
          ...
          <div className="p-1">
            ...
            >
              <div className={`w-8 h-8 ${isDark ? 'bg-gray-800' : 'bg-gray-100'} rounded-lg flex items-center justify-center`}>
                <Upload className={`w-5 h-5 ${isDark ? 'text-gray-400' : 'text-gray-600'} group-hover:text-blue-600 transition-colors`} />
              </div>
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-900'} group-hover:text-blue-600 transition-colors`}>
                  Upload Files
                </div>
            ...
            >
              <div className={`w-8 h-8 ${isDark ? 'bg-gray-800' : 'bg-gray-100'} rounded-lg flex items-center justify-center`}>
                <Database className={`w-5 h-5 ${isDark ? 'text-gray-400' : 'text-gray-600'} group-hover:text-blue-600 transition-colors`} />
              </div>
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-900'} group-hover:text-blue-600 transition-colors`}>
                  Connect Database
